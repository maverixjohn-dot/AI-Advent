...
def call_api(messages, *, model, temperature, max_tokens, fmt, api_key, timeout=180):
    payload = {
        "model": model,
        "messages": messages,
...
def run_variant(cfg, task, api_key):
    ...
    title = (f"{model} | {STRATEGIES[strategy]} | T={cfg['temperature']:.2f} | "
...
class VariantTab(ttk.Frame):
    ...
    def __init__(self, master, saved=None):
        ...
        ttk.Label(self, text="Температура:").grid(row=1, column=2, sticky="w", pady=(6, 0))
        t_frame = ttk.Frame(self)
        t_frame.grid(row=1, column=3, sticky="w", padx=4, pady=(6, 0))
        self.temp_var = tk.DoubleVar(value=saved.get("temperature", 1.0))
        self.temp_lbl = ttk.Label(t_frame, text=f"{self.temp_var.get():.2f}", width=5)
        self.temp_scale = ttk.Scale(t_frame, from_=0.0, to=2.0, length=140,
                                    variable=self.temp_var,
                                    command=lambda v: self.temp_lbl.config(
                                        text=f"{float(v):.2f}"))
        self.temp_scale.pack(side="left")
        self.temp_lbl.pack(side="left")

        # --- строка 2: системный промпт
        ttk.Label(self, text="Системный промпт:").grid(row=2, column=0, sticky="nw", pady=(6, 0))
        self.system = tk.Text(self, height=2, wrap="word")
        self.system.grid(row=2, column=1, columnspan=3, sticky="ew", padx=4, pady=(6, 0))
        ...
        s_frame.grid(row=3, column=0, columnspan=4, sticky="ew", pady=(8, 0))
        ...
        self.instr_frame.grid(row=4, column=0, columnspan=4, sticky="ew", pady=(8, 0))
        self.exp_frame.grid(row=5, column=0, columnspan=4, sticky="ew", pady=(8, 0))
        self._toggle()
